...
|:---:|:---:|
|출발 노드가 1개|모든 노드에 대하여 다른 모든 노드로 가는 정보 구함|
|1차원 리스트에 최단 거리 정보 저장|2차원 리스트에 최단 거리 정보 저장|
|그리디 알고리즘|다이나믹 프로그래밍|

### 작동 순서
- 현재 확인하고 있는 노드 제외한 N - 1 개의 노드 중 서로 다른 노드 (A, B)쌍 선택
- A→1번 노드→B로 가는 비용을 확인한 뒤 최단 거리 갱신
- (n-1)P(2) 개의 쌍을 단계마다 반복해서 확인
    - 이 단계에서의 시간 복잡도는 O(N^2)
    - 따라서 전체 시간 복잡도는 O(N^3)

### 특징
구체적인 (K번의 단계에 대한) 점화식: **D**ab = min(**D**ab, **D**ak + **D**kb)
</br>
'A에서 B로 가는 최소 비용'과 'A에서 K를 거쳐 B로 가는 비용'을 비교하여 더 작은 값으로 갱신하겠다는 의미
